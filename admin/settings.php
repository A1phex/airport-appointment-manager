...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($action == 'update_profile') {
        $stmt = $database->prepare("UPDATE admin SET name = ? WHERE id = ?");
        ...
        $stmt = $database->prepare("SELECT password_hash FROM admin WHERE id = ?");
        ...
        if (!password_verify($_POST['current_password'] ?? '', $account['password_hash'])) {
            ...
            $stmt = $database->prepare("UPDATE admin SET password_hash = ? WHERE id = ?");
...
$stmt = $database->prepare("SELECT * FROM admin WHERE id = ?");
$stmt->bind_param("i", $_SESSION['user_id']);
$stmt->execute();
